...
<div class="bread bread2">
	...
	<a href="${request.contextPath}/main/bsp.action" title="" >首页</a><em>&gt;</em>
		<a href="${request.contextPath}/noticeManage/queryNoticeList.action?currentFormId=aTurnForm&aTurnForm=a8046389-c87f-465a-b808-225e87a312d2&menuId4css=100000254&optFuntionId=100000416&optFuntionName=通知管理" title="" >通知管理</a>
	<em>&gt;</em><i>新增通知</i>
</div>
<!--面包屑 ]]-->
<!-- 主体内容 [[-->
<form name="insertNoticeform" id ="insertNoticeform" onsubmit="return titleIsEmpty();" action="${request.contextPath}/noticeManage/main/saveNotice.action" method="post">
<div class="main main2">
	<div class="case-scroll">
		<div class="case">
			<div class="case-top-title">新增通知</div>
			...
			<input type="hidden" name="noticeTypeValue" id="noticeTypeValue" value="4"/>
		...
		<table border="0" cellspacing="0" cellpadding="0" class="new-gonggao-table" style="table-layout:fixed;width:auto;">
		  <tr>
			<td class="w108">标题：</td>
			<td  width="900">
				<input type="text" name="title" id="title" maxlength="40" onblur="titleIsEmpty();" class="uitext w172"/><span id="titleSpan" class="tipTxt"></span>
		  ...
		  <tr>
			...
			<td>
				<div class="two-double">
					<input type="radio" name="noticeType" id="noticeType4" value="4" onclick="changeNoticeType(4);" checked /><label for="noticeType4">交易中心重要通知</label>
				</div>
				<div class="two-double">
					<input type="radio" name="noticeType" id="noticeType5" value="5" onclick="changeNoticeType(5);" /><label for="noticeType4">外联通知</label>
				</div>				
			</td>
		  </tr>
		  <tr id="trOutLink" style="display: none;">
			  <td class="w108">外链地址：</td>
			  <td>
			  	<input type="text"  id="outLink" name="outLink" onblur="titleIsEmpty();" class="uitext w172"/>		  	
			  </td>
		  </tr>
		  <tr>
...
<script type="text/javascript">
	...
	$(document).ready(function() { 
		...
		$("#file_upload").uploadify({
			...
			'uploader'        : '${request.contextPath}/noticeManage/uploadNoticeImg.htm;jsessionid=${jsessionid}',
			'buttonText'      : '上传',
    ...
    function titleIsEmpty() {
    	...
    	$("#titleSpan").empty();
    	if(title.length>40){
    		$("#titleSpan").append('<em class="tipFalse4"></em><i class="hongse">请输入少于40个汉字。</i>');
    	}
    	if (title == null || title == '') {
