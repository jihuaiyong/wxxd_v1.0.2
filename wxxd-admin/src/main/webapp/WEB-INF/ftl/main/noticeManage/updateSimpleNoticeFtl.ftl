...
<!--面包屑 ]]-->
<!-- 主体内容 [[-->
<form name="updateNoticeform" id ="updateNoticeform" onsubmit="return titleIsEmpty();" action="${request.contextPath}/noticeManage/main/updateSimpleNotice.action" method="post">
<div class="main main2">
		<div class="case-scroll">
		<div class="case">
			<div class="case-top-title">修改通知</div>
			...
			<input type="hidden" name="infoPath" value="${simpleNotice.infoPath!}"/>
		...
		<table border="0" cellspacing="0" cellpadding="0" class="new-gonggao-table">
		  ...
		  <tr>
			...
			<td>
				...
				<div class="two-double" <#if simpleNotice.noticeType != '5'>style="display:none"</#if>>
					<input type="radio" name="noticeType" id="noticeType5" value="5" onclick="changeNoticeType(5);" <#if simpleNotice.noticeType == '5'>checked</#if>/><label for="noticeType5">外联通知</label>
				</div>				
			</td>
		  </tr>
		  <#if simpleNotice.noticeType== '5'>
		  <tr id="trOutLink">
			  <td class="w108">外链地址：</td>
			  <td>
			  	<input type="text"  id="outLink" name="outLink" value="${simpleNotice.outLink!}" onblur="titleIsEmpty();" class="uitext w172"/>		  	
			  </td>
		  </tr>		  	
		  </#if>
		  <tr>
			...
			<td>
				...
				<#if simpleNotice.noticeType == '5'>
				<select id="showPlace" name="showPlace" class="zhanshiweizhi" onchange="changeplace(this.value);">
					<option value="3" <#if simpleNotice.showPlace == '3'>selected</#if>>新标签页中</option>
				</select>
				</#if>				
