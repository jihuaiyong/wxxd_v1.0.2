...
<div class="sucPath">
		...
		<div><span><@spring.message code="page.forward"/></span><input id="inputPageNum2" name="inputPageNum2" type="text" value="" size="2" onkeyup="this.value=this.value.replace(/\D/g,'')"/>页
		<input type="button" value="<@spring.message code='page.button'/>"  onclick="jumpPage($('#inputPageNum2').val(),'${toURL}')" class="pagesubmit">
		</div>
		<#--</from>-->
	</div> 
</div> 
