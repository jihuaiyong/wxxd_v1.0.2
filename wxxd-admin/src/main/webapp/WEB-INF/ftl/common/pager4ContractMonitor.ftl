...
<div class="sucPath">
		...
		<div><span><@spring.message code="page.forward"/></span><input id="inputPageNum1" name="inputPageNum1" type="text" value="" size="2" onkeyup="this.value=this.value.replace(/\D/g,'')"/>页
		<input type="button" value="<@spring.message code='page.button'/>"  onclick="jumpPage($('#inputPageNum1').val(),'${toURL}')" class="pagesubmit">
		</div>
		<#--</from>-->
	</div> 
</div> 
