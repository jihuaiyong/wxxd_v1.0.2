...
!function ($) {
	...
	bTabs.prototype.addTab = function(id,title,url,loginCheck){
		...
		var openIframe = function(){
			$(content).append('<iframe id="'+tabId+100+'" class="tabs-1" frameborder="0" fit="true" scrolling="yes" style="width:100%;height:100%;border:0px;" src="'+url+'"></iframe>');
		};
		//进行登录验证
	 ...
	 * 关闭当前标签页
	 */
	function NowbTabsClose(id){
		id = id == undefined ? this.find('li[class="active"]').find("a").attr("href").replace("#", "") : id;
		return this.each(function(){
			if(!id) return;
			var $this = $(this),data = $this.data('bTabs');
			if(data) data.closeTab(id);
	 ...
	 * 关闭其他标签页
	 */
	function OtherbTabsClose(){
		var $this = $(this),data = $this.data('bTabs');
		var tabArr=[];
		for(var k = 0; k < data.openTabs.length; k++){
			tabArr.push(data.openTabs[k]);
		}
		//data.openTabs.splice(0,data.openTabs.length);
		//data.openTabs.push(this.find('li[class="active"]').find("a").attr("href").replace("#", ""));
		//data.openTabs.push("bTabs_navTabsMainPage");
		removeByvalue(tabArr,"bTabs_navTabsMainPage");
		removeByvalue(tabArr,this.find('li[class="active"]').find("a").attr("href").replace("#", ""));
		//this.find(".nav-tabs li").not('[class="noclose"]');
		//this.find(".tab-content div").not('[id="bTabs_navTabsMainPage"]');
		//this.find(".nav-tabs li").not('[class="active"]').remove();		
		//this.find(".tab-content div").not('[class="tab-pane active"]').remove();		
		 for (var i = 0; i < tabArr.length; i++) {
			 data.closeTabNew1(tabArr[i]);
	        }
	}
	...
	function AllbTabsClose(){
		...
		removeByvalue(tabArr,"bTabs_navTabsMainPage");
		//		var d = '<ul class="nav nav-tabs" role="tablist"> <li role="presentation" class="active noclose"><a href="#bTabs_navTabsMainPage" data-toggle="tab">首页</a></li> </ul><div class="tab-content"><div class="tab-pane active" id="bTabs_navTabsMainPage"><div class="page-header"><h2 style="font-size: 31.5px;text-align: center;font-weight: normal;">欢迎使用价格管控系统</h2></div><div style="text-align: center;font-size: 20px;line-height: 20px;"> Welcome to use PMCP! </div></div></div>';
//		this.find(".nav-tabs li").remove();
//		this.find(".tab-content div").remove();
//		data.openTabs = new Array();
		//Plugin(p);
//	    location.reload();
//		return this.html(d);
		for (var i = 0; i < tabArr.length; i++) {
			data.closeTabNew(tabArr[i]);
	        }
	 ...
	 *  
	 **/
	bTabs.prototype.closeTabNew = function(id){
		var c = constants,$tabs = this.$container, openTabs = this.openTabs;
		//移除内容区
		$('#' + id).remove();
		var a = $('ul.nav-tabs a[href="#'+id+'"]',$tabs);
	 ...
	 *  
	 **/
	bTabs.prototype.closeTabNew1 = function(id){
		var c = constants,$tabs = this.$container, openTabs = this.openTabs;
		//移除内容区
		$('#' + id).remove();
		var a = $('ul.nav-tabs a[href="#'+id+'"]',$tabs);
	...
	$.fn.NowbTabsClose     = NowbTabsClose;
	$.fn.OtherbTabsClose   = OtherbTabsClose;
	$.fn.AllbTabsClose     = AllbTabsClose;
	$.fn.removeByvalue     = removeByvalue;
	// 处理新旧版本冲突
	// =================
