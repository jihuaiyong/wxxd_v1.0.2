...
function getAuthByPage(pageNumber) {
	...
	$.ajax({
		url : ctx + '/supplierInfo/showSupplierInfoData.action',
		type : "post",
			...
			if (lineData && lineData.total) {
				...
				for (var i = 0; i < len; i++) {
					var tempData = lineData.rows[i];
						...
						tabHTML += '<td>' + nullToEmpty(tempData.status) + '</td>';
...
function getQueryParams(pageNumber) {
	var supplier = $('#wrap .item-search .code-goods > .zg-inpMany > .zg-form-control').val();
	var supplierCodeArray = new Array();
	if(supplier){
		supplierCodeArray = supplier.split(",");
	}
	var supplier ="";
	var cas = "";
	for (var i = 0; i < supplierCodeArray.length; i++) {
		cas ="'"+ supplierCodeArray[i].trim() +"'";
		if (cas.length != 0) {
			if(supplier != ""){
