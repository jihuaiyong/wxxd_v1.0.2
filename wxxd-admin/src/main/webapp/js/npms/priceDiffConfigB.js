...
function add(){
    ...
    $("#diff_ratio").removeAttr("disabled") ;
    $("#addDialog").show();
};
...
function checkParams() {
    ...
    if (initData == 0) {
        repeatCheckFlag = checkRepeat(brandCode, diffRatio);
    }
    checkFlag = brandCodeFlag && diffRatioFlag && repeatCheckFlag;
    return checkFlag;
}


function checkRepeat(brandCode, diffRatio) {
    var repeatCheckFlag = true;
    $.ajax({
        type:"POST",
...
function commit(){
    ...
    if(initData==0){
        if (checkParams()) {
            $.ajax({
                type:"POST",
                url:"addPriceDiffB.action",
                data:{'brandCode':bcode,'diffRatio':dratio,'deleteFlag':delete_flag},
        ...
        $.ajax({
            type:"POST",
            url:"updatePriceDiffB.action",
            data:{'brandCode':bcode,
                'diffRatio':dratio,
...
function editDetail(obj){
    ...
    $("#diff_ratio").val(tds[3].innerHTML);
    initData=1;
    restSpan();
    $("#addDialog").show();
};
...
function downloadModel() {
    window.open('/npms-admin/modelFile/priceDiffB.zip','下载上传模板',"");
}
